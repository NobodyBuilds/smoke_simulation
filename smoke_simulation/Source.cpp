    ...
    for (int iter = 0; iter < iterations; iter++)
    {
        for (int y = 1; y < h - 1; y++)
        {
            for (int x = 1; x < w - 1; x++)
            {
                ...
                int flowtop = isolid(x, y + 1) ? 0 : 1;
                int flowbottom = isolid(x, y - 1) ? 0 : 1;
                int flowleft = isolid(x - 1, y) ? 0 : 1;
                int flowright = isolid(x + 1, y) ? 0 : 1;
                int dgecount = flowbottom + flowtop + flowleft + flowright;

                if (isolid(x, y) || dgecount == 0)
                {
                    pressure[i] = 0.0f;
                    continue;
                }

                float newpressure = ((pressure[i - 1] * flowleft) + (pressure[i + 1] * flowright) + (pressure[i - w] * flowtop) + (pressure[i + w] * flowbottom) - scale * Div[i] // ← correct sign + physical scaling
                                     ) /
                                    dgecount;
...
float sample(std::vector<float> &field, float x, float y)
{
    ...
    return (1 - tx) * (1 - ty) * field[y0 * w + x0] + tx * (1 - ty) * field[y0 * w + x1] + (1 - tx) * ty * field[y1 * w + x0] + tx * ty * field[y1 * w + x1];
}
    ...
    for (int iter = 0; iter < 20; iter++)
    {
        for (int y = 1; y < h - 1; y++)
        {
            for (int x = 1; x < w - 1; x++)
            {
                ...
                if (isolid(x, y))
                    continue;
                density[i] = (tmp[i] + a * (density[i - 1] + density[i + 1] +
